...
Tai lieu chi tiet cho flag nang cao, debug, va cac lenh phu.

Neu chi muon dung 2 pipeline chinh, doc `README.md`.
...
Hai lenh mac dinh cho user khong can truyen folder hay lesson number:
...
- `fetch-lesson` doc file tu `put_image_here_to_fetch_ans/`, copy vao `user_data/fetch_sessions/<session_name>/`, roi chay fetch answers.
- `submit-lesson` doc file tu `put_image_here_to_submit/`, copy vao `user_data/submit_sessions/<session_name>/`, roi upload len Drive.

Cac lenh ben duoi la lenh chi tiet/de debug khi can tu set folder hoac flag rieng.
...
- `listening`: ghep unit voi audio cung so, luu transcript debug, tao skeleton theo marker xanh, roi dien dap an theo block.
